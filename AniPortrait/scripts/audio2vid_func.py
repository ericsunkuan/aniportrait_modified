...
def a2v(a2m_model, a2p_model, pipe,lmk_extractor, vis, ref_image_path, audio_path, acc = True):
    ...
    # prepare model   (here we don't load a2m and a2p but we pass the it though function)
    ...
    args_L = len(pose_images) if L is None else L
    for pose_image_np in pose_images[: args_L]:
        pose_image_pil = Image.fromarray(cv2.cvtColor(pose_image_np, cv2.COLOR_BGR2RGB))
        pose_tensor_list.append(pose_transform(pose_image_pil))
    sub_step = fi_step if acc else 1
